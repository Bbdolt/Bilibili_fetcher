...
class MainFrame(tk.Frame):
    def __init__(self, root):
        ...
        # 配置正则表达式
        self.obj = re.compile(r'result_type:.*?,data:[.*?res:[(.*?)],', re.DOTALL)
        self.obj1 = re.compile(r'\{type:e.*?\}', re.DOTALL)
        self.title_obj = re.compile(r'title:"(.*?)",')
        self.views_obj = re.compile(r'play:"(.*?)"')
        self.length_obj = re.compile(r'duration:"(.*?)"')
        self.author_obj = re.compile(r'author:"(.*?)"')
        self.url_obj = re.compile(r'arcurl:"(.*?)"')
        # 在线播放线程
    ...
    def search(self):
        ...
        urls = []
        try:
            with requests.get(f'https://search.bilibili.com/video?keyword={keyword}&page={self.page}', headers=headers1111, verify=False) as r:
                html = etree.HTML(r.text)
                data1111 = html.xpath('/html/body/script[4]/text()')[0]
                print(data1111)
                data1111 = self.decode_unicode_escapes(data1111)
                result111 = self.obj.findall(data1111)
                result222 = self.obj1.findall(data1111)
                result333 = result111 + result222
                if result333:
                    for i in result333:
                        if 'http' in i:
                            names.append(self.title_obj.findall(i)[0].replace(r'<em class=\\"keyword\\">', "").replace(r'</em>', "").replace(r'<em class=\"keyword\">', "") if self.title_obj.findall(i) else None)
                            views.append(self.views_obj.findall(i)[0] if self.views_obj.findall(i) else None)
                            lengths.append(self.length_obj.findall(i)[0] if self.length_obj.findall(i) else None)
                            authors.append(self.author_obj.findall(i)[0] if self.author_obj.findall(i) else None)
                            urls.append(self.url_obj.findall(i)[0] if self.url_obj.findall(i) else None)
                            print(i)
                    # html_element = etree.HTML(r.text)
                    # print(r.text)
                    # parent_div = html_element.xpath('/html/body/script[4]/text()')
                    # parent_div2 = self.decode_unicode_escapes(parent_div[0])
                    # parent_div1 = self.obj.findall(parent_div2)
                    # print(parent_div)
                    # if parent_div:
